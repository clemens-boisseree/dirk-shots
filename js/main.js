...
function addIVWPixel() {
    var iam_data = {
        "st": "rponl",
        "cp": "RP-Thema-Vermischtes",
        "sv": "ke",
        "co": "interaktiv.rp-online.de/ig/19/dirk-nowizki-career-shotchart/"
    }
...
var map = L.map('map', {
    ...
    minZoom: 14,
    maxZoom: 20
}).setView([-0.0076806, -0.0022278], startzoom);
...
var maxBounds = L.latLngBounds(
    L.latLng(-0.0537642, 0.0155946),
    L.latLng(0.0039554, -0.0155946)
);
...
var url = 'https://interaktiv.rp-online.de/ig/19/dirk-nowitzki-career-shotchart/data/dirk-shots.json';
...
function process() {
    ...
    var shots = new L.TopoJSON(null, {
        ...
        filter: function (feature, layer) {
            return (!filter.shotresult || feature.properties.SM == filter.shotresult) &&
                (!filter.opponent || feature.properties.HTM == filter.opponent || feature.properties.VTM == filter.opponent) &&
                (!filter.S || feature.properties.S == filter.S) &&
                (!filter.gametype || feature.properties.GT == filter.gametype);
        }
    });
    $.getJSON(url, function (data) {
        shots.addData(data);
...
for (var i = 0; i < buzzer.length; i++) {
    buzzer[i].addEventListener('click', function (event) {
        theExpression = 'feature.properties.MR == "00:00" ';
        reset();
        buzzer = new L.TopoJSON(null, {
            ...
            filter: function (feature, layer) {
                return (feature.properties.MR == "00:00");
            }
        });
...
for (var i = 0; i < bestgame.length; i++) {
    bestgame[i].addEventListener('click', function (event) {
        theExpression = 'feature.properties.GD == "02.12.2004" ';
        reset();
        bestgame = new L.TopoJSON(null, {
            ...
            filter: function (feature, layer) {
                return (feature.properties.GD == "02.12.2004");
            }
        });
...
for (var i = 0; i < firstshot.length; i++) {
    firstshot[i].addEventListener('click', function (event) {
        theExpression = 'feature.properties.GI == "29800012" ';
        reset();
        firstshot = new L.TopoJSON(null, {
            ...
            filter: function (feature, layer) {
                return (feature.properties.GI == "29800012");
            }
        });
...
for (var i = 0; i < lastshot.length; i++) {
    lastshot[i].addEventListener('click', function (event) {
        theExpression = 'feature.properties.GI == "21801227" ';
        reset();
        lastshot = new L.TopoJSON(null, {
            ...
            filter: function (feature, layer) {
                return (feature.properties.GI == "21801227");
            }
        });
...
for (var i = 0; i < finals.length; i++) {
    finals[i].addEventListener('click', function (event) {
        theExpression = 'feature.properties.S == "2010-11" && feature.properties.GT == "P" && feature.properties.HTM == "MIA" || feature.properties.VTM == "MIA"';
        reset();
        finals = new L.TopoJSON(null, {
            ...
            filter: function (feature, layer) {
                return (feature.properties.S == "2010-11") && (feature.properties.GT == "P") && (feature.properties.HTM == "MIA" || feature.properties.VTM == "MIA");
            }
        });
        $.getJSON(url, function (data) {
            finals.addData(data);
...
$.getJSON('https://interaktiv.rp-online.de/ig/19/dirk-nowitzki-career-shotchart/data/court.topo.json').done(addTopoData);
function addTopoData(topoData) {
...
function renderTooltip(data) {
    ...
    tooltip.find(".Saison").text(data.S);
    tooltip.find(".Spiel").text(game);
    tooltip.find(".Datum").text(data.GD);
    tooltip.find(".Wurf").css("color", colorWurf);
    tooltip.find(".Wurf").text(shot);
    tooltip.find(".Entfernung").text((data.SD * 0.31).toFixed(2).toString().replace(".", ","));
    tooltip.find(".Typ").text(data.AT);
    tooltip.find(".Spielzeit").text(data.MR);
    tooltip.find(".Viertel").text(data.P);
    tooltip.find(".link a").attr('href', 'https://stats.nba.com/game/' + "00" + encodeURIComponent(data.GI));
    if (video) {
        document.getElementById('video').style.display = 'block';
        tooltip.find(".video-link iframe").attr('src', 'https://videos.nba.com/nba/pbp/media/'+video+'1280x720.mp4');
    }
    else {
    ...
    /*
        tooltip.find(".video-link a").attr('href', 'https://stats.nba.com/events/?flag=1&GameID=' + "00" + encodeURIComponent(data.GI) + '&GameEventID=' + encodeURIComponent(data.GAME_EVENT) + '&S=' + encodeURIComponent(data.S));
    */
}
